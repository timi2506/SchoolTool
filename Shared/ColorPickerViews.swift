...
// MARK: - Color picker views (iOS / macOS / tvOS only; watchOS has no color-picking UI)
...
struct ColorSheet: View {
    ...
    var colors: [Color] = [
        .red,
        Color(hue: 0.03, saturation: 0.8, brightness: 1.0),
        .orange,
        .yellow,
        .mint,
        .green,
        ...
        .indigo,
        .purple,
        Color(hue: 0.9, saturation: 0.4, brightness: 1.0),
        .pink,
        .brown,
        .darkGray,
        .gray,
    ...
    var body: some View {
        Button(action: { selection = color }) {
            Circle()
                ...
                .overlay { Circle().stroke(color.opacity(0.25), lineWidth: 2) }
                .padding(5)
                .overlay {
                    if selection == color {
...
extension View {
    func colorSheet(isPresented: Binding<Bool>, color: Binding<Color>) -> some View {
        modifier(ColorSheetModifier(isPresented: isPresented, color: color))
    }
}
    ...
    var body: some View {
        Button(action: {
            ...
            HStack {
                label
                Spacer()
                Circle().frame(width: 25, height: 25).foregroundStyle(color)
